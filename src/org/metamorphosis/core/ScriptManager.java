...
public class ScriptManager {
	...
	public GroovyScriptEngine createScriptEngine(File folder) throws Exception {
		URL[] urls = { folder.toURI().toURL(),
				new File(context.getRealPath("/") + "/" + SCRIPTS_FOLDER).toURI().toURL() };
				...
				try {
					...
					for (Object annotation : annotations) {
						...
						if (value.indexOf("WebServlet") != -1) {
							...
							clazz.setSuperclass(classPool.get("org.metamorphosis.core.ServletRequestListener"));
							return clazz.toBytecode();
						} else if (value.indexOf("ContextAttributeListener") != -1) {
							clazz.setSuperclass(
									classPool.get("org.metamorphosis.core.ServletContextAttributeListener"));
							return clazz.toBytecode();
						} else if (value.indexOf("RequestAttributeListener") != -1) {
							clazz.setSuperclass(
									classPool.get("org.metamorphosis.core.ServletRequestAttributeListener"));
							return clazz.toBytecode();
							...
							clazz.setSuperclass(classPool.get("org.metamorphosis.core.HttpSessionAttributeListener"));
							return clazz.toBytecode();
	...
	protected ImportCustomizer createCompilationCustomizer() {
		...
		importCustomizer.addStarImports("org.metamorphosis.core", "javax.servlet", "javax.servlet.annotation",
				"javax.servlet.http", "org.metamorphosis.core.annotation", "groovy.json");
